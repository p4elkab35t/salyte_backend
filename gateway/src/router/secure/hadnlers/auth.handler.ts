...
const routes = new Map<string, { method: string[], handler: Function }>([
  ...
  async function lifeCheck(req: Request, restPath: string) {
    ...
    return new corsResponse(JSON.stringify({ message: "Auth Service Alive" }), { status: 200 });
  }
  ...
  async function verifyToken(req: Request) {
    ...
    if (result.status !== 0){
      return new corsResponse(JSON.stringify({ error: "Verification failed" }), { status: 401 });
    }
    Logger.info("Token verified", { result });
    return new corsResponse(JSON.stringify(result), { status: 200 });
  } catch (error) {
    return new corsResponse(JSON.stringify({ error: {error} }), { status: 401 });
  }
  }
  ...
  async function signIn(req: Request) {
    ...
    if (req.method === "GET") {
      ...
      if (result.status !== 0){
        return new corsResponse(JSON.stringify({ error: "Sign in failed" }), { status: 401 });
      }
  ...
  async function signUp(req: Request) {
    ...
    if (result.status !== 0){
      return new corsResponse(JSON.stringify({ error: "Sign up failed" }), { status: 401 });
    }
  ...
  async function signOut(req: Request) {
    ...
    if (result.status !== 0){
      return new corsResponse(JSON.stringify({ error: "Sign out failed" }), { status: 401 });
    }
